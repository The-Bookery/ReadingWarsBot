...
module.exports.execute = async (client, message, args) => {
  ...
  if (!team) { // Only runs if userid is set, in other words if they're looking for a specific user
    if (userid == "712698434670297108") { // Special case for Finriq
      let helpMessage = new Discord.MessageEmbed().setColor('#750384').setTitle('Statistics for Finriq.').setDescription(`See granular statistics about this user's habits.`).addFields({
        name: `Points`,
        value: `\`\`\`∞\`\`\``,
        inline: true
      }, {
        ...
        value: `\`\`\`∞\`\`\``,
        inline: true
      }, ).addField(`Class`, `\`\`\`Finriq\`\`\``).addField(`Coins`, `\`\`\`∞\`\`\``).addFields({
        name: 'Read',
        value: `\`\`\`∞\`\`\``,
        inline: true
      }, {
    ...
    pomMembers.sync().then(() => {
      try {
        pomMembers.findAll({
          attributes: ["id", "user", [Sequelize.literal('RANK () OVER ( ORDER BY points DESC )'), 'rank']],
          raw: true,
        }).then((rankresult) => {
          pomMembers.findAll({
            where: {
            ...
            if (result.length == 1) {
              ...
              let helpMessage = new Discord.MessageEmbed().setColor('#750384').setTitle('Statistics for ' + member.user.username + '.').setDescription(`See granular statistics about this user's habits.`).addFields({
                ...
                name: `Team`,
                value: `\`\`\`${result[0].team[0].toUpperCase() + result[0].team.slice(1)}\`\`\``
              }, {
                name: `Class`,
                ...
                name: 'Trade In',
                value: `\`\`\`${result[0].tradein}\`\`\``,
                inline: true
              }, {
                name: 'Attack',
                value: `\`\`\`${result[0].attack}\`\`\``,
                inline: true
              }, {
                name: 'Build',
                value: `\`\`\`${result[0].build}\`\`\``,
                inline: true
              }, {
                name: 'Bomb',
                value: `\`\`\`${result[0].bomb}\`\`\``,
    ...
    try {
      pomTeams.findAll({
        attributes: ["id", "team", [Sequelize.literal('RANK () OVER ( ORDER BY points DESC )'), 'rank']],
        raw: true,
      }).then((rankresult) => {
        pomTeams.findAll({
          where: {
          ...
          let helpMessage = new Discord.MessageEmbed().setColor('#750384').setTitle('Statistics for team ' + wordteam + '.').setDescription(`See statistics about this team's habits.`).addFields({
            ...
            value: `\`\`\`${rank}\`\`\``,
            inline: true
          }, ).addField('Member Total', '```Total: ' + memberCount + '```').addField('Member Breakdown', `\`\`\`Knights: ${result[0].knights}
Thieves: ${result[0].thieves}
Stonemasons: ${result[0].stonemasons}
Jokers: ${result[0].jokers}\`\`\``).addFields({
            name: 'Read',
            value: `\`\`\`${result[0].read}\`\`\``,
            inline: true
          }, {
