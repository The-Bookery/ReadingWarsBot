...
module.exports.execute = async (client, message) => {
  new rm.menu(message.channel, message.author.id, [
    ...
    .setTitle('(#1) Basics')
    .setDescription(
      `The reading wars are fun and competitive events where three teams battle for supremacy on the leaderboard. To climb up the leaderboard, you need to earn coins and trade them in for points using a variety of commands. To earn one coin, you have to read for 15 minutes. You can, however, time yourself and read for longer, then divide the minutes by 15 and enter them all at once. With those coins, you can attack other teams and steal their points, defend your team from attacks by building walls, bomb other people's walls, and more.

You can also join a class, which will give you different buffs to play to your specific playstyle. The bot will automatically assign you to one of the three teams. Join in and fight your hardest! After a month and a half, the team with the most points wins!`
    )
    .addField(
      'Turn the Page!',
      'To turn the page, react to this message with `▶`.'
    ),
    new Discord.MessageEmbed()
    .setColor('#750384')
    .setTitle('(#2) Commands')
    ...
    .addFields(
      ...
      { name: `\`b-bomb <team number>\``, value: `This command does three damage to whichever team's walls you target. It is more more efficient than the attack command, therefore, because it is 100% consistent and does far more damage to walls than attacks do. However, it does not earn you any points.`}
    )
    .addField(
      'Turn the Page!',
      'To turn to the next page, react to this message with `▶`. To go back, react with `◀`.'
    ),
    new Discord.MessageEmbed()
    .setColor('#750384')
    .setTitle('(#3) Classes')
    ...
    .addFields(
      ...
      'Turn the Page!',
      'To go back to the first page, react to this message with `⏪`. To go back one, react with `◀`. To stop this menu, react with `⏹`.'
    )
