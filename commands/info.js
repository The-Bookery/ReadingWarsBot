...
module.exports.execute = async (client, message) => {
  new rm.menu(message.channel, message.author.id, [
    ...
    .addFields(
      { name: `\`b-read [times]\``, value: `This command should be run after doing 15 minutes of reading. Feel free to add a number afterward (e.g. \`b-read 3\`) for a certain amount of 15-minute reading times. Be sure to actually time yourself while doing this (you could, for example, set a 2 hour timer and then use \`b-read 8\`, for example). Stay honest or you could get banned from the event and even future events.` },
